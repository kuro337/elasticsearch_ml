...
function sortPostsByScores(posts, scores) {
  return posts.sort((a, b) => {
    const scoreA = scores.find((score) => score.post_id === a.post_id)?.score || 0;
    const scoreB = scores.find((score) => score.post_id === b.post_id)?.score || 0;
...
], Product.prototype, "manufacturers", undefined);
__legacyDecorateClassTS([
  html("date", "Enter last accessed date")
], Product.prototype, "last_accessed", undefined);
...
], Post.prototype, "component", undefined);
__legacyDecorateClassTS([
  html("text", "Enter dynamic path")
], Post.prototype, "dynamic_path", undefined);
__legacyDecorateClassTS([
  html("text", "Enter render function")
], Post.prototype, "render_func", undefined);
...
], Interaction.prototype, "interaction_type", undefined);
__legacyDecorateClassTS([
  html("text", "Enter post id")
], Interaction.prototype, "post_id", undefined);
__legacyDecorateClassTS([
  html("text", "Enter username")
], Interaction.prototype, "username", undefined);
...
function handleSimilarEntitiesResponse(data) {
  ...
  const entityIndexName = Object.keys(data.data.results).find((key) => key !== "scores");
  if (!entityIndexName) {
  ...
  const entities = rawEntities.map((rawEntity) => createEntity(entityStr, rawEntity));
  ...
  document.dispatchEvent(scoresEvent);
  console.log(`Similar Entities Event Dispatched :\n eventName : ${entityIndexName}Received`);
}
...
function createEntityFromInputs(EntityClass, inputContainerSelector) {
  const inputs = document.querySelectorAll(`${inputContainerSelector} sl-input`);
...
function renderEntitiesOnSimilarityEvent(divId, entityType) {
  document.addEventListener(`${entityType}Received`, (event) => {
    ...
    const cardsHtml = entities.map((entity) => generateEntityCard(entity, entityType)).join("");
