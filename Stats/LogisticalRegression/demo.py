...
from LogisticalRegression.mock_data.mock_users import users
from LogisticalRegression.mock_data.mock_posts import posts
from LogisticalRegression.mock_data.gen_feature_set import simulate_interactions
...
from LogisticalRegression.prepare.prepare_data import prepare_dataset_glm
from LogisticalRegression.scoring.train_model import train_and_evaluate_config
from LogisticalRegression.scoring.score import (
    score_data,
    map_scores_to_data,
    get_scores,
)
from LogisticalRegression.cache.save_model import (
    save_model,
    print_model_coefficients,
)


def train_model_get_scores(model_path: str = None) -> List[UserPostScore]:
    """
    @Demo

    - Trains Model
    - Returns User-Score Pairs for Each User-Post
    """
    print(
    ...
    # @Merge
    merged_df_one_encode, original_data = prepare_dataset_glm(state, copy=True)

    # @Retain
    # Cloning Series for Retention for Scoring and Mapping

    retained_columns = extract_columns_for_retention(merged_df_one_encode, state=state)
    ...
    scores = get_scores(final_data_scored, state)
